...
import { useUploadComplianceDoc, useVendorSummary } from '../api/hooks';
...
export function ComplianceDocs() {
  ...
  const uploadDoc = useUploadComplianceDoc();
    ...
    uploadDoc.mutate(
      {
        ...
        type: docType,
        expiryDate,
        replaceId: replacingId ?? undefined,
      },
      {
        onSuccess: () => {
        ...
        <div className="p-4.5">
          ...
          <div className="flex gap-2 mt-3.5">
            <button
              ...
              className="bg-och-teal hover:bg-och-teal-dark disabled:opacity-60 text-white text-[13px] font-semibold px-4 py-2 rounded-md transition-colors"
            >
              {uploadDoc.isPending
                ? 'Uploading…'
                : replacingId
